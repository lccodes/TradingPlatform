...
\mydef{Lemma} \\
Consider a non-myopic strategy that is dominant for an agent in a MSR based market. That agent has a final belief, which incorporates whatever logic and prior information the agent has, called $\Agent_\AgentBelief$.  
...
shareNum = 0;
\While{$Market$.getPriceYes(shareNum) $< \Agent_\AgentBelief$}{
	shareNum += .01;
}
$\Market$.buyYes(shareNum);
}{
shareNum = 0;
\While{$\Market$.getPriceNo(shareNum) $> \Agent_\AgentBelief$}{
	shareNum += .01;
}
\Market.buyNo(shareNum);
}
\end{algorithm}
