...
\subsection{Metrics}
<<<<<<< HEAD
For each of the following configurations, we will rank our three market makers against our two benchmarks defined above: \mydef{Profit-Maximizing Market Maker} and \mydef{Accuracy} according to RE, PI, and PIT. For our experiments, we will assume that private beliefs are ${0,1}$ and that budgets are equal.\\
=======
For each of the following configurations, we will rank our three market makers against our two benchmarks defined above: \mydef{Profit-Maximizing Market Maker} and \mydef{Accuracy} according to RE and PI.
>>>>>>> newInterfaces
...
=======
\item \mydef{Liquidity Sensitive Market Maker} with varying alpha parameters. Since alpha corresponds to the commission, we will assess all $\LMSRAlpha$ values on the interval [0,1] with two digits of precision. This evaluates the full range of logical values.

\item \mydef{Time Sensitive Market Maker} with varying $\LMSRAlpha$ parameters. Since $\LMSRAlpha$ corresponds to the commission, we will assess all $\LMSRAlpha$ values on the interval [0,1] with two digits of precision. This evaluates the full range of logical values.
>>>>>>> newInterfaces
\end{enumerate}
...
the market. Our novel definition attempts to quantity consistency across \mydef{market depths} without
...
metric. After all the simulations, the \mydef{market makers} will be ranked for each metric depending on the empirically
