...
\SetAlgoLined
\TitleOfAlgo{cost}
\KwData{market maker $\MarketMaker$, new quantity $\Quantity$, direction $\Option_\OptionDirection$}
\KwResult{cost $\Money$}
initialization\;
...
\eIf{direction == YES}{
	newScore = $\MarketMaker_\LMSRb * \exp(\frac{\MarketMaker_\QuantityYes + \Quantity}{\MarketMaker_\LMSRb}) + \exp(\frac{\MarketMaker_\QuantityNo}{\MarketMaker_\LMSRb})$\;
}{
	newScore = $\MarketMaker_\LMSRb * \exp(\frac{\MarketMaker_\QuantityYes}{\MarketMaker_\LMSRb}) + \exp(\frac{\MarketMaker_\QuantityNo + \Quantity}{\MarketMaker_\LMSRb})$\;
}
...
\KwData{market maker $\MarketMaker$, desiredPrice $\OptionPrice$, direction boolean}
\KwResult{quantity $\Quantity$}
initialization\;
\eIf{direction}{
price = $\Market_\MarketPrice$\;
side = $\MarketMaker_\QuantityYes$\;
top = $\MarketMaker_\QuantityNo$\;
}{
price = (1-$\Market_\MarketPrice$)\;
side = $\MarketMaker_\QuantityNo$\;
top = $\MarketMaker_\QuantityYes$\;
}
