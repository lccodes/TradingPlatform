\section{Problem}
The liquidity parameter $\LMSRb$ both controls how quickly that agents can change the market price and bounds the loss that the market maker can incur. Our algorithm will deal with this trade-off by minimizing the the market maker's loss while constraining the market's final price to equal the collective belief within a small margin of error.
...
We define the aggregate belief according to the Rational Expectations Equilibrium.\\

The \mydef{Rational Expectations Equilibrium} implies that the final price will be the average of each agent's belief since all agents have equal budgets under our model. Formally: $\OptionPrice_\EventTime = \frac{\sum_{i=1}^{|\Agents|} \Agent^{i}_{\AgentBelief}}{|\Agents|}$. \\
