...
public class TestServer extends AgentServer {
	...
	protected void onRegistration(Connection connection, Registration registration) {
		...
		this.theServer.sendToTCP(connection.getID(), new PMRegistration(theID, Math.random() > .5 ? C : D));

		Account oldAccount = bank.get(connections.get(connection));
		Security newSec = null;
		// if (Math.random() < .5 && !yes) {
		newSec = new Security(oldAccount.ID, 1, TYPEYES, state -> state.getState() == 1 ? new Account(null).add(100)
				: null);
		//yes = true;
